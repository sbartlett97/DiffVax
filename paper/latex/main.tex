...
\begin{figure*}[t]
  ...
  \caption{\textbf{Overview of main results.}
    \textbf{(a)}~Patch-based 1088$\times$1088 inference: overlapping 512px patches accumulate
    perturbation, yielding 1.60$\times$ baseline EDR with no retraining.
    \textbf{(b)}~Cross-architecture transfer: the SD1.5-only checkpoint transfers to FLUX.1-schnell
    and SD3.5 zero-shot; multi-model training backfires (H1a row is weaker on all columns).
    \textbf{(c)}~JPEG paradox on FLUX: compression at q=75 increases the SD1.5-only checkpoint's
    EDR by 50\% (from 0.200 to 0.300); the effect disappears for the multi-model and
    JPEG-augmented checkpoints, showing the paradox is an emergent property of strong,
    single-objective perturbations.}
  \label{fig:overview}
\end{figure*}
...
JPEG is an established purifier against pixel-space immunization for UNet-based
diffusion models~\citep{zhao2023jpegbypass}; this ICLR 2025 community consensus
~\citep{vanle2025cannot} treats JPEG as an adversary to be defended against.
Our FLUX finding inverts this consensus.
GAP-Diff~\citep{gapdiff2025} achieves JPEG-robust facial protection via JPEG
simulation backpropagation; we find this approach (H7) fails for immunization
at q=70--75 due to perturbation weakening. \citet{bengio2013ste} introduced the STE.

\textbf{JPEG and transformer sensitivity.}
\citet{li2022jpegvit} show ViT patch embeddings natively absorb JPEG DCT blocks,
with block artifacts surviving intact into token space; CNNs do not share this property.
\citet{guo2023rspc} show ViT patch-corruption sensitivity is structurally severe.
\citet{chen2024transformadv} model JPEG as an adversarial trigger (+17--31\% in black-box).
These three papers collectively explain why our JPEG paradox is architecture-specific to FLUX.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig3_patch_accumulation}
  \caption{\textbf{Patch accumulation mechanism.}
    \textit{Left}: overlap count heatmap for 1088px image at stride=256. The center pixel
    is covered by 4 patches (white star), accumulating 4$\times$ the adversarial signal.
    \textit{Right}: EDR and PSNR vs.\ inference strategy (same NestedUNet checkpoint throughout).
    Both effectiveness (EDR) and imperceptibility (PSNR$\geq$28~dB) are satisfied at 50\% overlap.}
  \label{fig:patch}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig2_jpeg_paradox}
  \caption{\textbf{JPEG paradox across architectures and checkpoints.}
    For FLUX.1-schnell (center), the SD1.5-only checkpoint's EDR \emph{increases} by 50\%
    at q=75 and 30\% at q=70. The effect is absent on SD1.5 (UNet, convolutional) and
    smaller on SD3.5 (MM-DiT). Multi-model and JPEG-augmented checkpoints show no paradox,
    confirming the effect requires strong single-objective perturbations.}
  \label{fig:jpeg}
\end{figure}
...
\textbf{Training-duration confound.} A potential concern is that H1a ran for only
10 epochs (${\approx}16{,}000$ gradient steps, batch\_size=1) while the original DiffVax
ran for 350 epochs with batch\_size=5 (${\approx}112{,}000$ gradient steps, or ${\sim}3.5\times$ more
effective updates when accounting for the larger batch). We observe that H1a's training
loss stabilised by epoch~10, suggesting it did not under-converge; however, a matched-compute
comparison (H1a at batch\_size=5 for 350 epochs) would be the cleanest test.
We note that the gradient competition mechanism is structural (loss magnitude imbalance every step,
independent of total training time) and predict it would persist under matched compute.
H1b (gradient normalization via per-model loss scaling) addresses the root cause rather than the symptom.
...
\textbf{Why this is architecture-specific.} Three converging lines of evidence from
the literature support the patch-token explanation:
(1)~\citet{guo2023rspc} show that corrupting as few as 10\% of ViT patches causes
severe accuracy drops through global attention propagation --- FLUX's $2\times2$
patch tokenization creates an identical structural vulnerability.
(2)~\citet{li2022jpegvit} demonstrate that ViT patch embeddings natively absorb JPEG's
$8\times8$ DCT block representation, with DCT artifacts surviving intact into token space;
CNNs require architectural surgery to even accept DCT inputs and smooth block boundaries naturally.
(3)~\citet{chen2024transformadv} show that JPEG quality-factor can serve as an adversarial
trigger that \emph{amplifies} pixel-space perturbations by 17--31\% in black-box scenarios,
with the effect being architecture-specific. Our JPEG paradox is consistent with this model.

SD1.5's convolutional UNet processes images through multi-scale spatial filters
that smooth block artifacts before they can disrupt the denoising prior~\citep{pierson2024pixel}.
This explains why JPEG has no effect on SD1.5 EDR while FLUX EDR increases 50\%.
Prior work confirms that JPEG is an effective \emph{purifier} against SD1.5-based
immunization~\citep{zhao2023jpegbypass,vanle2025cannot}; our FLUX result is a qualitatively
distinct, novel exception to this consensus.

\textbf{Why STE training cannot amplify the paradox.} The paradox requires a strong
base perturbation: H1a (+7\%) and H7 (none) show diminishing effect correlating with
weaker PSNR. The STE signal rewards perturbation directions that \emph{survive} JPEG,
but the compound effect depends on the perturbation being strong enough to interact with
DCT artifacts --- a condition STE training actively undermines by diverting optimization
energy toward compression-survivor frequencies. GAP-Diff~\citep{gapdiff2025} also
addresses JPEG robustness via JPEG-simulation backpropagation, but for facial customization
(different task and architecture); it does not leverage the DCT--DiT compounding
mechanism we describe here, since its target model is a UNet-based diffusion model.
